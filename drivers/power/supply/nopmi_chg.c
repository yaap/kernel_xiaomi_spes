					...
					const union power_supply_propval *val)
{
	...
	rc = vote(nopmi_chg->fcc_votable, CHG_INPUT_SUSPEND_VOTER,
			(bool)val->intval, 0);
	if (rc < 0) {
					    ...
					    const union power_supply_propval *val)
{
	...
	if (nopmi_chg->system_temp_level == 0) {
		...
		rc = vote(nopmi_chg->fcc_votable,
				THERMAL_DAEMON_VOTER, true, nopmi_chg->thermal_mitigation[nopmi_chg->system_temp_level] / 1000); //divide 1000 to match maxim driver fcc as mA
	}
			      ...
			      const union power_supply_propval *val)
{
	...
	pr_err("2021.09.21 wsy %s psp=%d, val->intval=%d",
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		break;
	case POWER_SUPPLY_PROP_MTBF_CUR:
		pr_err("%s psp=%d Set MTBF current val->intval=%d",
				__func__, psp, val->intval);
...
static void  nopmi_cv_step_monitor_work(struct work_struct *work)
{
	...
	pr_err("fg_cc_cv_step_check: batt_volt=%d, batt_curr=%d",
			batt_volt, batt_curr);
	/*discharging*/
