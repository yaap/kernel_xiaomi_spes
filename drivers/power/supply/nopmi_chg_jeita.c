...
static void nopmi_chg_handle_jeita_current(struct nopmi_chg_jeita_st *nopmi_chg_jeita)
{
	...
	if (!g_ffc_disable && fast_charge_mode
		...
		power_supply_set_property(nopmi_chg_jeita->bms_psy,
				POWER_SUPPLY_PROP_FASTCHARGE_MODE, &prop);
	} else if (!g_ffc_disable && !fast_charge_mode
		&& (sw_jeita->sm == TEMP_T2_TO_T3)) {
		prop.intval = 1;
		fast_charge_mode = 1;
		power_supply_set_property(nopmi_chg_jeita->bms_psy,
	...
	if (sw_jeita->cv != chg1_cv) {
		if (nopmi_chg_jeita->fv_votable) {
			vote(nopmi_chg_jeita->fv_votable, JEITA_VOTER, true,
					sw_jeita->cv);
		} else {
			ret = nopmi_chg_jeita_set_charger_voltage(nopmi_chg_jeita,
					sw_jeita->cv);
			if (ret < 0)
...
static void nopmi_chg_handle_jeita(struct nopmi_chg_jeita_st *nopmi_chg_jeita)
{
	...
	if (nopmi_chg_jeita->sw_jeita->charging == false) {
		...
		case NOPMI_CHARGER_IC_MAXIM:
			vote(nopmi_chg_jeita->chgctrl_votable, JEITA_CHG_VOTER,
...
int nopmi_chg_jeita_init(struct nopmi_chg_jeita_st *nopmi_chg_jeita)
{
	...
	pr_err("2021.09.10 wsy %s: start init\n", __func__);
	if (!nopmi_chg_jeita->sw_jeita) {
		nopmi_chg_jeita->sw_jeita = kmalloc(sizeof(struct sw_jeita_data),
				GFP_KERNEL);
		if (!nopmi_chg_jeita->sw_jeita) {
...
int nopmi_chg_jeita_deinit(struct nopmi_chg_jeita_st *nopmi_chg_jeita)
{
	...
	pr_err("2021.09.10 wsy %s: start deinit\n", __func__);
	if (!nopmi_chg_jeita->sw_jeita) {
