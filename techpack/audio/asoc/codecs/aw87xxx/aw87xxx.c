...
static int aw87xxx_update_off_prof(struct aw87xxx *aw87xxx, char *profile)
{
	...
	prof_desc = aw_acf_get_prof_desc_form_name(aw87xxx->dev,
	...
	if (aw_dev->hwen_status == AW_DEV_HWEN_OFF) {
		AW_DEV_LOGI(aw87xxx->dev, "profile[%s] has already load",
				profile);
	} else {
...
int aw87xxx_update_profile(struct aw87xxx *aw87xxx, char *profile)
{
	...
	prof_desc = aw_acf_get_prof_desc_form_name(aw87xxx->dev,
			&aw87xxx->acf_info, profile);
	if (prof_desc == NULL) {
		AW_DEV_LOGE(aw87xxx->dev, "not found [%s] parameter",
				profile);
		mutex_unlock(&aw87xxx->reg_lock);
		return -EINVAL;
	}
	...
	AW_DEV_LOGD(aw87xxx->dev, "load profile[%s] succeed",
			profile);
	return 0;
}
...
int aw87xxx_show_current_profile_index(int dev_index)
{
	...
	AW_LOGE("not found struct aw87xxx, dev_index = [%d], index = [%d]",
			dev_index, index);
	return index;
}
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	if (!profile_name) {
		AW_DEV_LOGE(aw87xxx->dev, "not found profile name,index=[%d]", index);
		return -EINVAL;
	}
...
static int aw87xxx_kcontrol_dynamic_create(struct aw87xxx *aw87xxx, void *codec)
{
	...
	kctl_name[0] = devm_kzalloc(aw87xxx->dev, AW_NAME_BUF_MAX, GFP_KERNEL);
	if (kctl_name[0] == NULL)
		return -ENOMEM;
	...
	kctl_name[1] = devm_kzalloc(aw87xxx->codec->dev, AW_NAME_BUF_MAX, GFP_KERNEL);
	if (kctl_name[1] == NULL)
		return -ENOMEM;
	...
	if (ret < 0) {
		AW_DEV_LOGE(aw87xxx->dev, "add codec controls failed, ret = %d", ret);
		return ret;
	}
...
static void aw87xxx_fw_load_retry(struct aw87xxx *aw87xxx)
{
	...
	AW_DEV_LOGD(aw87xxx->dev, "failed to read [%s]", aw87xxx->fw_name);

	if (acf_info->load_count < AW_LOAD_FW_RETRIES) {
		AW_DEV_LOGD(aw87xxx->dev, "restart hrtimer to load firmware");
		schedule_delayed_work(&aw87xxx->fw_load_work,
				msecs_to_jiffies(ram_timer_val));
	} else {
		acf_info->load_count = 0;
		AW_DEV_LOGE(aw87xxx->dev, "can not load firmware,please check name or file exists");
		return;
	}
	acf_info->load_count++;
}
...
{
	struct aw87xxx *aw87xxx = container_of(work, struct aw87xxx, fw_load_work.work);
	struct aw_prof_info *prof_info = &aw87xxx->acf_info.prof_info;
	...
	if (prof_info->status == AW_ACF_WAIT) {
		request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
				aw87xxx->fw_name, aw87xxx->dev,
				GFP_KERNEL,
				aw87xxx, aw87xxx_fw_load);
	}
}
...
static void aw87xxx_fw_load_init(struct aw87xxx *aw87xxx)
{
	...
	schedule_delayed_work(&aw87xxx->fw_load_work, msecs_to_jiffies(cfg_timer_val));
}
			...
			struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < aw_dev->reg_max_addr; i++) {
		...
		if (ret < 0) {
			...
			len += snprintf(buf + len, PAGE_SIZE - len,
					"reg:0x%02X=0x%02X\n", i, reg_val);
			AW_DEV_LOGD(aw87xxx->dev, "reg:0x%02X=0x%02X", i, reg_val);
		}
	}
	mutex_unlock(&aw87xxx->reg_lock);
			...
			const char *buf, size_t count)
{
	...
	if (count < AWRW_HDR_LEN) {
		...
		AW_DEV_LOGE(aw87xxx->dev, "string %s,count=%ld", buf, (u_long)count);
		return -EINVAL;
	}
...
static struct aw87xxx *aw87xxx_malloc_init(struct i2c_client *client)
{
	...
	aw87xxx = devm_kzalloc(&client->dev, sizeof(struct aw87xxx), GFP_KERNEL);
	if (aw87xxx == NULL) {
